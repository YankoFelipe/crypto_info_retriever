...
class MovingAverageGenerator:
    ...
    def fill_table(self, has_spec: bool = False, is_resuming: bool = False):
        ...
        if has_spec:
            self.spec.id = self.moving_averages_repo.get_spec_id(self.spec)
        else:
            self.spec.id = self.moving_averages_repo.save_spec(self.spec)

        if is_resuming:
            print('Preparing to resume')
            self.current_time = self.moving_averages_repo\
                .get_first_time_of_last_average(self.spec) - self.spec.candle_in_seconds()
        ...
        self.ma_calculator = MovingAverageCalculator(self.spec,
                                                     self.current_time)
        print('Ready to fill!')
        while self.price_stream.is_alive():
            if self.price_stream.time() % self.spec.candle_in_seconds() == 0:
